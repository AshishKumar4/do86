...
const IMAGES: Record<string, ImageDef> = {
  ...
  aqeous:     { file: "aqeous.bin",              drive: "multiboot", memory: WASM_MB, vgaMemory: VGA_MB, label: "AqeousOS", noSnapshot: true, ahciDiskSize: 32, logicalMemory: 96 },
  ...
  private async bootVM(): Promise<void> {
    ...
    try {
      ...
      const v86Config: Record<string, any> = {
        ...
        // workerd blocks runtime WASM compilation (WebAssembly.instantiate on
        // dynamically generated bytes throws CompileError).  JIT can never work
        // here, so disable it to avoid wasted CPU on hotness tracking / codegen.
        disable_jit: true,
        bios: { buffer: bios },
