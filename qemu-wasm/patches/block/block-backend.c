...
{
#ifdef __EMSCRIPTEN__
    /* On Emscripten, blk_nb_sectors uses coroutines which fail with ASYNCIFY
     * during callMain. Get file size directly from MEMFS instead. */
    int64_t size = -1;
    /* Try /disk.img (floppy) and /disk.iso (cdrom) */
    size = EM_ASM_INT({
        try {
            var s1 = Module.FS.stat('/disk.img');
            if (s1 && s1.size > 0) return s1.size;
        } catch(e) {}
        try {
            var s2 = Module.FS.stat('/disk.iso');
            if (s2 && s2.size > 0) return s2.size;
        } catch(e) {}
        return -1;
    });
    if (size > 0) {
        *nb_sectors_ptr = size / BDRV_SECTOR_SIZE;
        return;
    }
