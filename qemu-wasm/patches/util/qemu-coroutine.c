...
void qemu_aio_coroutine_enter(AioContext *ctx, Coroutine *co)
{
    ...
    while (!QSIMPLEQ_EMPTY(&pending)) {
        ...
        if (scheduled) {
            ...
            fprintf(stdout, "[CORO] already-sched func='%s' from=%p to=%p\n", scheduled, from, to); fflush(stdout);
#endif
            abort();
        }
...
#ifdef __EMSCRIPTEN__
            fprintf(stdout, "[CORO] recursive from=%p to=%p caller=%p\n", from, to, to->caller); fflush(stdout);
#endif
            abort();
        }
...
void coroutine_fn qemu_coroutine_yield(void)
{
    ...
    if (!to) {
#ifdef __EMSCRIPTEN__
        /* In Emscripten with ASYNCIFY Fibers, coroutines entered from the
         * leader context have no caller to yield TO. Instead of aborting,
         * switch to the leader coroutine which returns to the caller of
         * qemu_coroutine_enter(). */
        to = qemu_coroutine_self();
        if (to == self) {
            /* We ARE the leader — can't yield to ourselves.
             * Just return — the operation completed synchronously. */
            self->caller = NULL;
            return;
        }
#else
